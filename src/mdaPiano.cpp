...
#include "mdaPianoData.generated.h"
#include "mdaPiano.h"
...
#define PIANO_INDEX(value) ((value) / PIANO_SAMPLE_DECIMATION)
#define PIANO_START(value) (((value) + PIANO_SAMPLE_DECIMATION - 1) / PIANO_SAMPLE_DECIMATION)
#define PIANO_LOOP(end, loop) (PIANO_INDEX(end) + 1 - PIANO_INDEX((end) + 1 - (loop)))
...
mdaPiano::mdaPiano(audioMasterCallback audioMaster) : AudioEffectX(audioMaster, NPROGS, NPARAMS)
{
  ...
  kgrp[ 0].root = 36;  kgrp[ 0].high = 37;
  kgrp[ 1].root = 40;  kgrp[ 1].high = 41;
  kgrp[ 2].root = 43;  kgrp[ 2].high = 45;
  kgrp[ 3].root = 48;  kgrp[ 3].high = 49;
  kgrp[ 4].root = 52;  kgrp[ 4].high = 53;
  kgrp[ 5].root = 55;  kgrp[ 5].high = 57;
  kgrp[ 6].root = 60;  kgrp[ 6].high = 61;
  kgrp[ 7].root = 64;  kgrp[ 7].high = 65;
  kgrp[ 8].root = 67;  kgrp[ 8].high = 69;
  kgrp[ 9].root = 72;  kgrp[ 9].high = 73;
  kgrp[10].root = 76;  kgrp[10].high = 77;
  kgrp[11].root = 79;  kgrp[11].high = 81;
  kgrp[12].root = 84;  kgrp[12].high = 85;
  kgrp[13].root = 88;  kgrp[13].high = 89;
  kgrp[14].root = 93;  kgrp[14].high = 999;
  #define PIANO_KGRP(group_index, source_pos, source_end, source_loop) \
    kgrp[group_index].pos = PIANO_START(source_pos); \
    kgrp[group_index].end = PIANO_INDEX(source_end); \
    kgrp[group_index].loop = PIANO_LOOP(source_end, source_loop)
  PIANO_KGRP(0, 0, 36275, 14774);
  PIANO_KGRP(1, 36278, 83135, 16268);
  PIANO_KGRP(2, 83137, 146756, 33541);
  PIANO_KGRP(3, 146758, 204997, 21156);
  PIANO_KGRP(4, 204999, 244908, 17191);
  PIANO_KGRP(5, 244910, 290978, 23286);
  PIANO_KGRP(6, 290980, 342948, 18002);
  PIANO_KGRP(7, 342950, 391750, 19746);
  PIANO_KGRP(8, 391752, 436915, 22253);
  PIANO_KGRP(9, 436917, 468807, 8852);
  PIANO_KGRP(10, 468809, 492772, 9693);
  PIANO_KGRP(11, 492774, 532293, 10596);
  PIANO_KGRP(12, 532295, 560192, 6011);
  PIANO_KGRP(13, 560194, 574121, 3414);
  PIANO_KGRP(14, 574123, 586343, 2399);
  #undef PIANO_KGRP
...
void mdaPiano::noteOn(int32_t note, int32_t velocity)
{
  ...
  if(velocity>0)
  {
    ...
    l = 22050.0f * iFs * (float)exp(0.05776226505 * l) / PIANO_SAMPLE_DECIMATION;
