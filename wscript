...
def options(opt):
    ...
    opt.add_option('--epiano-resampling', type='int', default=1, dest='epiano_resampling',
                   help='Keep every Nth EPiano PCM sample (1, 2, 3, or 4; default: 1)')
    opt.add_option('--piano-resampling', type='int', default=1, dest='piano_resampling',
                   help='Keep every Nth Piano PCM sample (1, 2, 3, or 4; default: 1)')

def configure(conf):
    is_pico = getattr(conf.options, 'picolv2', False)
    resampling_factors = {
        'EPiano': conf.options.epiano_resampling,
        'Piano': conf.options.piano_resampling,
    }
    for plugin, factor in resampling_factors.items():
        if factor not in (1, 2, 3, 4):
            conf.fatal('--%s-resampling must be 1, 2, 3, or 4' % plugin.lower())
    conf.env['PICOLV2'] = is_pico
    conf.env['EPIANO_RESAMPLING'] = str(resampling_factors['EPiano'])
    conf.env['PIANO_RESAMPLING'] = str(resampling_factors['Piano'])
    if is_pico:
...
def build(bld):
    ...
    sample_data = {
        'EPiano': ('mdaEPianoData.h', 'mdaEPianoData.generated.h', 'EPIANO_RESAMPLING'),
        'Piano': ('mdaPianoData.h', 'mdaPianoData.generated.h', 'PIANO_RESAMPLING'),
    }
    generated_headers = {}
    for plugin, (source_name, generated_name, resampling_env) in sample_data.items():
        generated_headers[plugin] = generated_dir.make_node(generated_name)
        bld(
            rule='"%s" "%s" ${SRC} ${TGT} %s' %
                 (sys.executable, generator, bld.env[resampling_env]),
    ...
    for p in plugins:
        ...
        if p in sample_data:
            defines.append('%s_SAMPLE_RESAMPLING=%s' %
